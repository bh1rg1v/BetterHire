...
export default function PositionEdit() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        await api.api(`/organizations/me/positions/url/${positionUrl}`, { method: 'PATCH', body });
      } catch (e) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.api(`/organizations/me/positions/url/${positionUrl}`),
      api.api('/organizations/me/managers'),
      api.api('/organizations/me/forms').catch(() => ({ forms: [] })),
      api.api('/organizations/me/tests').catch(() => ({ tests: [] })),
    ]).then(([posRes, managersRes, formsRes, testsRes]) => {
      const p = posRes.position;
      setForm({
        title: p.title,
        positionUrl: p.positionUrl || '',
        description: p.description || '',
        keyResponsibilities: Array.isArray(p.keyResponsibilities) ? p.keyResponsibilities.join('\n') : '',
        qualifications: Array.isArray(p.qualifications) ? p.qualifications.join('\n') : '',
        preferredQualifications: Array.isArray(p.preferredQualifications) ? p.preferredQualifications.join('\n') : '',
        requiredSkills: Array.isArray(p.requiredSkills) ? p.requiredSkills.join('\n') : '',
        preferredSkills: Array.isArray(p.preferredSkills) ? p.preferredSkills.join('\n') : '',
        jobType: p.jobType || 'Full Time',
        workEnvironment: p.workEnvironment || 'Onsite',
        compensation: Array.isArray(p.compensation) ? p.compensation.join('\n') : p.compensation || '',
        companyOverview: p.companyOverview || '',
        status: p.status,
        assignedManagerId: p.assignedManagerId?._id || '',
        formId: p.formId?._id || '',
        testId: p.testId?._id || '',
        openDate: p.openDate ? new Date(p.openDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        closeDate: p.closeDate ? new Date(p.closeDate).toISOString().split('T')[0] : '',
  ...
  const s = {
    ...
    autoSave: { fontSize: '0.875rem', color: theme.colors.textMuted, marginBottom: theme.spacing.md },
  ...
  if (loading) return <DashboardLayout sidebar={<Sidebar items={getNavItems()} />}><p>Loading...</p></DashboardLayout>;
  if (!form) return <DashboardLayout sidebar={<Sidebar items={getNavItems()} />}><p style={s.error}>{error || 'Position not found'}</p></DashboardLayout>;

  return (
    <DashboardLayout sidebar={<Sidebar items={getNavItems()} />}>
      <div style={s.container}>
        <div style={s.left}>
          <h1 style={s.title}>Edit Position</h1>
          ...
          <form onSubmit={handleSubmit} style={s.form}>
            ...
            <div style={s.formRow}>
              ...
              <div style={s.formGroup}>
                ...
                <input type="text" value={form.positionUrl} onChange={(e) => setForm((f) => ({ ...f, positionUrl: e.target.value }))} required style={s.input} />
              </div>
            </div>
            <div style={s.formRow}>
              <div style={s.formGroup}>
