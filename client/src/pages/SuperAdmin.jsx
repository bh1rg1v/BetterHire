...
export default function SuperAdmin() {
  ...
  const s = {
    ...
    tabs: { display: 'flex', gap: theme.spacing.md, marginBottom: theme.spacing.xl },
    tab: (active) => ({ padding: `${theme.spacing.md} ${theme.spacing.lg}`, background: active ? theme.colors.primary : theme.colors.bgHover, border: `1px solid ${active ? theme.colors.primary : theme.colors.border}`, color: active ? '#fff' : theme.colors.text, cursor: 'pointer', fontFamily: theme.fonts.body, fontWeight: 500 }),
  ...
  return (
    <DashboardLayout sidebar={<Sidebar items={navItems} />}>
      <div style={s.header}>
        <h1 style={s.title}>Super Admin Dashboard</h1>
      </div>
      <div style={s.tabs}>
        <button type="button" onClick={() => setView('users')} style={s.tab(view === 'users')}>Users ({users.length})</button>
        <button type="button" onClick={() => setView('organizations')} style={s.tab(view === 'organizations')}>Organizations ({organizations.length})</button>
      </div>
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u._id}>
                    ...
                    <td style={s.td}>
                      <span style={s.badge(u.role === 'SuperAdmin' ? '#dc2626' : u.role === 'Admin' ? '#3b82f6' : u.role === 'Manager' ? '#8b5cf6' : '#10b981')}>
                        {u.role}
                      </span>
                    </td>
                    <td style={s.td}>{u.organizationId?.name || '-'}</td>
                    <td style={s.td}>
                      <span style={s.badge(u.isActive ? '#10b981' : '#ef4444')}>
