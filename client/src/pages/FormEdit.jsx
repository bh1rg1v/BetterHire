...
export default function FormEdit() {
  const { formUrl: formUrlParam } = useParams();
  const { user } = useAuth();
  ...
  const [originalData, setOriginalData] = useState({ name: '', formUrl: '', fields: [] });
  ...
  useEffect(() => {
    api.api(`/organizations/me/forms/url/${formUrlParam}`)
      .then((res) => {
        const formName = res.form.name;
        const formUrlValue = res.form.formUrl || '';
        const loadedFields = res.form.schema?.fields || [];
        const defaultFieldIds = ['firstName', 'lastName', 'email', 'phone', 'degree', 'fieldOfStudy', 'institution', 'graduationYear', 'resume'];
        const updatedFields = loadedFields.map(f => {
          const isDefault = defaultFieldIds.includes(f.id);
          return { ...f, isDefault, isCustom: !isDefault && !['linkedin', 'portfolio', 'coverLetter'].includes(f.id) };
        });
        setName(formName);
        setFormUrl(formUrlValue);
        setFields(updatedFields);
        setOriginalData({ name: formName, formUrl: formUrlValue, fields: updatedFields });
      })
  ...
  function handleReset() {
    ...
    setFormUrl(originalData.formUrl);
  ...
  async function handleSubmit(e) {
    ...
    try {
      const schema = { fields };
      await api.api(`/organizations/me/forms/url/${formUrlParam}`, { method: 'PATCH', body: { name: name.trim(), formUrl: formUrl.trim() || undefined, schema } });
  ...
  const s = {
    ...
    success: { color: theme.colors.success, marginBottom: theme.spacing.lg, padding: theme.spacing.md, background: `${theme.colors.success}20`, borderRadius: '8px' },
  ...
  return (
    <DashboardLayout sidebar={<Sidebar items={getNavItems()} />}>
      <div style={s.container}>
        <div style={s.left}>
          <h1 style={s.title}>Edit Application Form</h1>
        ...
        <div>
          ...
          <input type="text" value={formUrl} onChange={(e) => setFormUrl(e.target.value)} style={s.input} placeholder="e.g., software-engineer-form" />
        </div>
          ...
          <div key={field.id} style={s.fieldCard}>
              ...
              <div style={s.fieldRow}>
                <div>
                  ...
                  <select value={field.type} onChange={(e) => updateField(index, 'type', e.target.value)} style={s.select}>
